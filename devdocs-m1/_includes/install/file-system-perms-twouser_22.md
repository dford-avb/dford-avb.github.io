...
To put the Openmage file system owner in the web server's primary group (assuming the typical Apache group name for CentOS and Ubuntu), enter the following command as a user with `root` privileges:
...
For example, to add the user `openmage_user` to the `apache` primary group on CentOS:
